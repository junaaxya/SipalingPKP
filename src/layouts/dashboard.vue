...
const userProfile = computed(() => ({
  ...
  avatar: resolveFileUrl(appStore.user?.avatarUrl)
}))
...
const navigationItems = computed(() => {
  if (appStore.isMasyarakat) {
    ...
    return operationalNavigationItems.value.filter((item) => allowed.has(item.value))
  }

  if (appStore.isSuperAdmin) {
    return operationalNavigationItems.value.filter((item) => item.value !== 'form-self')
  }
  ...
  if (appStore.isVerifikator) {
    return operationalNavigationItems.value.filter((item) => {
      if (item.permission) {
        ...
        if (permissions.some((permission) => String(permission).includes(':create'))) {
          return false
        }
...
const filteredNotifications = computed(() => {
  ...
  return notifications.value.filter((item) => item.category === notificationCategory.value)
})
...
const handleOpenNotification = async (item) => {
  ...
  if (item.link) {
    ...
    if (appStore.isAdminDesa && item.link.includes('/infrastructure-data')) {
      try {
        const url = new URL(item.link, window.location.origin)
        const surveyId = url.searchParams.get('surveyId') || url.searchParams.get('id')
        if (surveyId) {
        ...
        // Fall back to default navigation if URL parsing fails
      }
    }
